...
#   C = TMT-MS PTGDS vs MMSE — flat (biphasic signal is tissue-restricted)
#
...
pA <- ggplot(cor_data, aes(x = TMT_log2, y = Soma_log2)) +
  ...
  labs(title    = "PTGDS between platforms (weak between-platform correlation, r ~ 0.15)",
       x        = "TMT-MS (log2)",
       y        = "SomaScan (log2)",
...
pB <- ggplot() +
  ...
  labs(title = "LCN2/NEFL rise with decline (both platforms)",
       x     = "MMSE score",
       y     = "Z-score increase",
...
pC <- ggplot(tmt_c, aes(x = MMSE)) +
  ...
  labs(title = "TMT-MS PTGDS vs MMSE (flat; biphasic is tissue-restricted, cf. Fig.1)",
