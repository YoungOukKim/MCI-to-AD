# ==============================================================================
# FigS6_neuronal_subtypes.R
#
# Purpose : Generate Figure S6 — Differential vulnerability across
...
#     FigS6_variance.csv     — NDUFS1 variance by subtype
#     FigS6_bcl2.csv         — SST+ BCL2 trajectory
#     FigS6_ratio.csv        — SST+ BAX/BCL2 ratio trajectory
...
# Usage : Rscript analysis/FigS/FigS6_neuronal_subtypes.R
...
# ------------------------------------------------------------------------------
DATA_DIR <- Sys.getenv("FIGS6_DATA", unset = "data/SEA-AD")
...
df_prop  <- fread(file.path(DATA_DIR, "FigS6_proportions.csv"))
var_data <- fread(file.path(DATA_DIR, "FigS6_variance.csv"))
sst_bcl2 <- fread(file.path(DATA_DIR, "FigS6_bcl2.csv"))
sst_ratio <- fread(file.path(DATA_DIR, "FigS6_ratio.csv"))
...
# )
# write.csv(df_prop_out, file.path(DATA_DIR, "FigS6_proportions.csv"), row.names = FALSE)
#
# # NDUFS1 variance
...
# )
# write.csv(var_out, file.path(DATA_DIR, "FigS6_variance.csv"), row.names = FALSE)
#
# # BCL2 trajectory
# sst_bcl2_out <- neurons[cell_type_label == "SST_Neuron",
#                         .(BCL2_mean = mean(BCL2, na.rm=TRUE)), by=bin_rounded][order(bin_rounded)]
# write.csv(sst_bcl2_out, file.path(DATA_DIR, "FigS6_bcl2.csv"), row.names = FALSE)
#
# # BAX/BCL2 ratio
...
# write.csv(sst_ratio_out, file.path(DATA_DIR, "FigS6_ratio.csv"), row.names = FALSE)
#
