...
# Fig2A_I_panels.R
#
# Purpose : Generate individual Fig. 2 panels (A-I), NO panel labels.
...
#   Fig2F_qPCR.png
#   Fig2G_ELISA.png
#   Fig2H_correlation.png
#   Fig2I_schematic.png
#
# Panel mapping (Panel C locomotor removed):
...
#   H = ptgdsb.1 vs ngfr inverse correlation
#   I = Molecular recovery schematic
#
# Paper : Kim Y et al. (2025). https://github.com/YoungOukKim/MCI-to-AD
# Usage : Rscript analysis/Fig2_zebrafish/Fig2A_I_panels.R
...
# ------------------------------------------------------------------------------
# Paths  (override with environment variables or edit defaults below)
...
# Helper: single-marker WIF bar plot
...
# Helper: load, trim, scale; optionally add scale bar
load_wif <- function(fname, scalebar = TRUE) {
  ...
  if (scalebar) {
    bar_w <- 160L   # ~1000 um at this scale
    bar_h <- 5L
...
# Add group label directly onto MERGE column
...
# Column header strip  (total width = 800 * 3 = 2400 px)
...
qPCR_long <- bind_rows(q14, q21) %>%
  ...
  mutate(
    ...
    Group = factor(Group, levels = c(
      "Control", "MCI", "MCI+BXP-101\n(0.4 \u03bcg/ml)")),
    dpf  = factor(dpf,  levels = c("14 dpf", "21 dpf")),
...
# ==============================================================================
# PANEL H — ptgdsb.1 vs ngfr inverse correlation
# ==============================================================================
message(">>> Panel H ...")

q14_w <- read.csv(file.path(DATA_DIR, "qPCR_14dpf.csv"),
                  stringsAsFactors = FALSE) %>%
  filter(Replicate == "Average") %>%
  select(Gene, Control, MCI,
         MCI_BXP101_0.3ugml, MCI_BXP101_0.4ugml, MCI_BXP101_0.6ugml)

ptgds1 <- q14_w %>%
  filter(Gene == "ptgdsb.1") %>%
  pivot_longer(-Gene, names_to = "Group", values_to = "ptgdsb1") %>%
  select(-Gene)

ngfr_v <- q14_w %>%
  filter(Gene == "ngfr") %>%
  pivot_longer(-Gene, names_to = "Group", values_to = "ngfr") %>%
  select(-Gene)

corr_df <- inner_join(ptgds1, ngfr_v, by = "Group") %>%
  mutate(Label = dplyr::recode(Group,
    "Control"            = "Control",
    "MCI"                = "MCI",
    "MCI_BXP101_0.3ugml" = "BXP 0.3",
    "MCI_BXP101_0.4ugml" = "BXP 0.4",
    "MCI_BXP101_0.6ugml" = "BXP 0.6"))

r_val <- round(cor(corr_df$ptgdsb1, corr_df$ngfr), 3)
...
pH <- ggplot(corr_df,
             aes(x = ptgdsb1, y = ngfr, color = Label, label = Label)) +
  geom_point(size = 4) +
  geom_smooth(method = "lm", color = "gray40", linewidth = 0.8,
              linetype = "dashed", se = FALSE) +
  geom_text(nudge_y = 0.012, size = 5) +
  annotate("text",
           x = min(corr_df$ptgdsb1) + 0.01,
           y = max(corr_df$ngfr) - 0.01,
           label = paste0("r = ", r_val),
           size = 6, fontface = "italic", hjust = 0) +
  scale_color_manual(values = col_c) +
  labs(x = "ptgdsb.1 relative expression",
       y = "ngfr relative expression") +
  BASE

ggsave(file.path(OUT_DIR, "Fig2H_correlation.png"), pH,
       width = 5, height = 6, dpi = DPI, bg = WHITE)
message("  Saved Fig2H_correlation.png")

# ==============================================================================
# PANEL I — Molecular recovery schematic
# ==============================================================================
message(">>> Panel I ...")
...
pI <- ggplot(nodes, aes(x = x, y = y)) +
  annotate("segment",
           x    = c(1.28, 2.28, 3.28, 4.28),
           xend = c(1.72, 2.72, 3.72, 4.72),
           y = 1, yend = 1,
           arrow = arrow(length = unit(0.22, "cm")),
           color = "gray30", linewidth = 0.9) +
  geom_point(aes(color = col), size = 35, alpha = 0.12) +
  geom_point(aes(color = col), size = 35, shape = 1, linewidth = 1.2) +
  geom_text(aes(label = label, color = col),
            size = 6.0, fontface = "bold", lineheight = 0.9) +
...
ggsave(file.path(OUT_DIR, "Fig2I_schematic.png"), pI,
       width = 12, height = 4.0, dpi = DPI, bg = WHITE)
message("  Saved Fig2I_schematic.png")
