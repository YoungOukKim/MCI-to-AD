...
#   output/TableS8_Demographics.csv        →  Additional file 1: Table S8
#   output/Fig3_CSF_associations.csv       ->  Fig. 3 supporting data (CSF associations)
#   Fig. 3 is generated by Fig5_CSF_concordance.R
#
# Analysis notes:
#   CSF biomarker associations with cognition are evaluated against a NEFL
#   positive control. Incremental value of the PTGDS/LCN2 ratio is assessed by
#   10-fold cross-validation, and conversion by a pre-specified baseline-MCI
#   Cox model. This script reports CSF associations and the positive control,
...
write.csv(table_s6, file.path(OUT_DIR, "TableS8_Demographics.csv"),
          row.names = FALSE)

# ══════════════════════════════════════════════════════════════════════════════
# 7. CSF analysis — associations + positive control (NEFL)
# ══════════════════════════════════════════════════════════════════════════════
...
table_s7 <- do.call(rbind, assoc)
cat("\n  CSF marker associations (|r|<0.2 => not a clinical predictor):\n")
print(table_s7, row.names = FALSE)
write.csv(table_s7, file.path(OUT_DIR, "Fig3_CSF_associations.csv"), row.names = FALSE)

# ══════════════════════════════════════════════════════════════════════════════
# 8. Incremental value — 10-fold CV AUC
# ══════════════════════════════════════════════════════════════════════════════
message(">>> [8/9] Cross-validated incremental value of PTGDS/LCN2 ratio...")
...
message(sprintf("  CV-AUC full(with ratio) = %.3f | marker-only(no ratio) = %.3f | increment = %+.3f",
                auc_full, auc_base, auc_full - auc_base))
message(sprintf("  CV-AUC ratio-only = %.3f  (~0.5 = chance => ratio is not an independent predictor)",
                auc_ratio))

# ══════════════════════════════════════════════════════════════════════════════
# 9. Conversion analysis — baseline-MCI, pre-specified log-ratio
# ══════════════════════════════════════════════════════════════════════════════
...
message("\n", strrep("=", 65))
message("  ADNI summary: NEFL positive control + associations.")
message("  No AUC/HR prediction claim. Fig.5 -> Fig5_CSF_concordance.R")
message(strrep("=", 65))
